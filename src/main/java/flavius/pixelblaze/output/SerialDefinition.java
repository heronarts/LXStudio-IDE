 ...
 *
 * Retains a mapping of serial port names to {@link processing.serial.Serial} objects, so that it
 * can enforce a single object per name, to prevent port busy errors.
 */
public class SerialDefinition {
  ...
  private static final Logger logger = Logger.getLogger(SerialDefinition.class.getName());

  public SerialDefinition(String portName, int baudRate, int dataBits, int stopBits, int parity) throws IllegalArgumentException {
  ...
  public SerialDefinition(String portName, int baudRate) throws IllegalArgumentException {
  ...
  public static void requireValidPortName(String portName) throws IllegalArgumentException {
    if (!isValidPortName(portName)) {
      throw new IllegalArgumentException("a valid portNames is required. Valid portNames: " + String.join(",", validPortNames()));
    }
  }
  ...
  public void write(final byte[] message) throws IOException{
    ...
    if(!isConnected(this.portName)) {
      if (isValidPortName(this.portName)) {
        try {
          if(serialPort.port.isOpened()) serialPort.port.closePort();
          serialPort.port = new SerialPort(this.portName);
          if(!serialPort.port.isOpened()) serialPort.port.openPort();
          serialPort.port.setParams(this.baudRate, this.dataBits, this.stopBits, this.parity);
    ...
    try {
      // Find another cross-platform way to detect if a port is disconnected 🙄
      if(serialPort.port.isRING()) {
        setConnected(this.portName);
        throw new IOException(String.format("serial port %s is disconnected", this.portName));
      }
      serialPort.port.writeBytes(message);
    } catch (SerialPortException e) {
      setDisconnected(this.portName);
      throw new IOException(String.format("can't write message: %s", e.toString()));
    }
  }
}
